...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('python-smell-detector.showReport', () => {
        ...
        const panel = vscode.window.createWebviewPanel('smellReport', `Smell Report: ${path.basename(editor.document.fileName)}`, vscode.ViewColumn.Beside, {
            enableScripts: true
        });
        ...
        const pythonProcess = (0, child_process_1.spawn)(pythonPath, ['-u', pythonScriptPath, '--html', editor.document.fileName], {
        ...
        pythonProcess.on('error', (err) => {
            ...
            panel.webview.html = getErrorContent(`Failed to start 'python' process. \n\nError: ${err.message}\n\nPlease ensure 'python' is installed and added to your system PATH.`);
        });
...
function runSmellDetection(context, document, collection) {
    // Strategy 1: Try to find script relative to the extension installation folder
    const extensionRoot = context.extensionUri.fsPath;
    let pythonScriptPath = path.join(extensionRoot, '..', 'smelly_python', 'main.py');
    // Strategy 2: Hardcoded path for this specific environment (Fallback)
    ...
    const pythonProcess = (0, child_process_1.spawn)(pythonPath, ['-u', pythonScriptPath, '--json', document.fileName], {
